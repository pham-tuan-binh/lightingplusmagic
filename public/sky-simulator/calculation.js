$(document).ready(() => {
  function CalculateZenitalAbsolutes(turbidity, solar_zenith) {
    ...
    let Y0 =
      (4.0453 * turbidity - 4.971) *
        Math.tan((4 / 9 - turbidity / 120) * Math.PI) -
      0.2155 * turbidity +
      2.4192;
  ...
  function Yxy_to_RGB(Y, x, y) {
    ...
    let color = {
      r: gamma_correct(3.2406 * X - 1.5372 * Y - 0.4986 * Z) || "0",
      g: gamma_correct(-0.9689 * X + 1.8758 * Y + 0.0415 * Z) || "0",
      b: gamma_correct(0.0557 * X - 0.204 * Y + 1.057 * Z) || "0",
    };
  ...
  function getColor(time) {
    ...
    let total_hours_today = time % 24;

    let time_str =
      (Math.floor(total_hours_today) < 10
        ? "0" + Math.floor(total_hours_today)
  ...
  function exportFile() {
    ...
    for (let i = 0; i < 24; i += 1.5) {
      let time_str =
        ...
        ":" +
        (Math.floor((i - Math.floor(i)) * 60) < 10
          ? "0" + Math.floor((i - Math.floor(i)) * 60)
          : Math.floor((i - Math.floor(i)) * 60));
